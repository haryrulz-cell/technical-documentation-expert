...
async function generateDocx(docTitle, docType, docContent, userName) {
  ...
  const hCell = (text, w) => new TableCell({
    ...
    children: [new Paragraph({ spacing: { after: 0 }, children: [new TextRun({ text: String(text || ""), font: "Arial", size: 20, bold: true, color: WHITE })] })]
  });
  ...
  function parseInline(text) {
    ...
    return parts.filter(Boolean).map(part => {
      if (part.startsWith("**") && part.endsWith("**"))
        return new TextRun({ text: part.slice(2, -2), font: "Arial", size: 22, bold: true, color: DGRAY });
      if (part.startsWith("*") && part.endsWith("*"))
        return new TextRun({ text: part.slice(1, -1), font: "Arial", size: 22, italics: true, color: DGRAY });
      if (part.startsWith("`") && part.endsWith("`"))
        return new TextRun({ text: part.slice(1, -1), font: "Courier New", size: 20, color: "185FA5" });
  ...
  function parseMarkdownToDocx(md) {
    ...
    if (inTable) flushTable();
    return elements.length ? elements : [new Paragraph({ children: [new TextRun({ text: " " })] })];
  }
          ...
          new Table({
            ...
            rows: [
              ...
              ...[
                ...
                ["Version", "1.0 — DRAFT"],
                ["Date", date],
                ["Prepared By", userName || "Technical Documentation Expert"],
                ["Reviewed By", "[Client Solution Architect — TBC]"],
...
export async function POST(request) {
  try {
    ...
    if (!rawText) {
      return Response.json({ content: [{ type: "text", text: "No response received. Please try again." }] });
    }
    ...
    if (parsed) {
      ...
      try {
        docxBase64 = await generateDocx(docTitle, docType, docContent, userName || "");
      } catch (docErr) {
        console.error("DOCX generation error:", docErr.message);
        docError = docErr.message;
      }

      const safeFilename = (docTitle || "document")
        .replace(/[^a-z0-9\s]/gi, "")
        .trim()
        .replace(/\s+/g, "_")
        .slice(0, 60) + "_v1.0.docx";
      ...
      if (docxBase64) {
        responsePayload.document = { base64: docxBase64, filename: safeFilename, docType, docTitle };
      } else if (docError) {
        responsePayload.docError = docError;
      }
